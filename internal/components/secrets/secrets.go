...
type SecretsView struct {
    *tview.Table
    ...
    SelectVaultHandler           func(idx int)
...
func (sv *SecretsView) AddSecretsControls() {
    sv.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
        ...
        if event.Rune() == 'd' {
            row, col := sv.GetSelection()
            if cell := sv.GetCell(row, col); cell != nil && cell.Text != "" {
                sv.SelectedSecretChangedHandler(cell.Text, sv.CurrentKeyvaultName, sv.CurrentKeyvaultSubscription)
            }
            return nil
        }
        if event.Rune() >= '1' && event.Rune() <= '9' {
            if sv.SelectVaultHandler != nil {
                sv.SelectVaultHandler(int(event.Rune() - '1'))
            }
            return nil
        }
        return event
